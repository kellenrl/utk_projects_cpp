...
void set_newfriends(vector< vector<int> > &friends, vector< vector<int> > &new_friends) {
	...
	for (int i=0; i<N; i++) {
		for(vector<int>::iterator iti = friends[i].begin(); iti != friends[i].end(); iti++) {
			for(vector<int>::iterator itj = friends[*iti].begin(); itj != friends[*iti].end(); itj++) {
				if(*itj != i) { 
					int match = 0;
					for(vector<int>::iterator itk = friends[i].begin(); itk != friends[i].end(); itk++) {
						if(*itk == *itj) {
							match++;	
						}
					}
					if(match == 0) {
						new_friends[i].push_back(*itj);
					}
				}
			}
		}
...
void writetofile(const char *fname, vector<string> &name, vector< vector<int> > &friends) {
	...
	for (int i=0; i<N; i++) {
		...
		tempit = unique(friends[i].begin(), friends[i].end());
		friends[i].resize(tempit - friends[i].begin());
